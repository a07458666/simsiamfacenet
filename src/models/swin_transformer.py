...
class WindowAttention(nn.Module):
    ...
    def extra_repr(self) -> str:
        return f"dim={self.dim}, ws={self.window_size}, nh={self.num_heads}"
...
class SwinTransformerBlock(nn.Module):
    ...
    def extra_repr(self) -> str:
        return (
            f"dim={self.dim},ir={self.input_resolution}, nh={self.num_heads}, "
            f"ws={self.window_size}, ss={self.shift_size}, mr={self.mlp_ratio}"
        )
...
class BasicLayer(nn.Module):
    ...
    Args:
        ...
        qk_scale (float | None,
        optional): Override default qk scale of head_dim ** -0.5 if set.
        drop (float, optional): Dropout rate. Default: 0.0
    ...
    def extra_repr(self) -> str:
        return f"dim={self.dim}, in_re={self.input_resolution}, d={self.depth}"
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        ), f"Size ({H}*{W}) notMatch ({self.img_size[0]}*{self.img_size[1]})."
